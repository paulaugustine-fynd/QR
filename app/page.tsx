...
function Dashboard({ user, onSignOut }: { user: string; onSignOut: () => void }) {
  ...
  return <div className="app-shell">
    <aside className={mobileNavOpen ? "mobile-open" : ""}><div className="sidebar-head"><div className="brand-lockup dark"><FyndMark inverse/><span>Fynd QR</span></div><button className="sidebar-close" aria-label="Close menu" onClick={() => setMobileNavOpen(false)}><X/></button></div><nav><button className={view === "create" ? "active" : ""} onClick={() => changeView("create")}><Plus size={19}/> Create QR</button><button className={view === "library" ? "active" : ""} onClick={() => changeView("library")}><QrCode size={19}/> My QR codes <span className="count">{saved.length}</span></button></nav><div className="side-note"><div className="infinity">∞</div><strong>Unlimited. Forever.</strong><p>Your QR codes never expire and have no scan limits.</p></div><button className="profile" onClick={onSignOut}><span className="avatar">{user[0].toUpperCase()}</span><span><strong>{user.split("@")[0]}</strong><small>{user}</small></span><LogOut size={17}/></button></aside>
